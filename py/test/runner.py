...
class OpenfoodfactsTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            OpenfoodfactsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = OpenfoodfactsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = OpenfoodfactsTestRunner.getenv("OPENFOODFACTS_TEST_LIVE")
        override = OpenfoodfactsTestRunner.getenv("OPENFOODFACTS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = OpenfoodfactsTestRunner.getenv(key)
        ...
        explain = OpenfoodfactsTestRunner.getenv("OPENFOODFACTS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["OPENFOODFACTS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if OpenfoodfactsTestRunner._test_control is not None:
            return OpenfoodfactsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                OpenfoodfactsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            OpenfoodfactsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = OpenfoodfactsTestRunner.load_test_control()
...
def load_env_local():
    OpenfoodfactsTestRunner.load_env_local()


def env_override(m):
    return OpenfoodfactsTestRunner.env_override(m)


def entity_data(v):
    return OpenfoodfactsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return OpenfoodfactsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return OpenfoodfactsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return OpenfoodfactsTestRunner.load_test_control()


def live_client_options():
    return OpenfoodfactsTestRunner.live_client_options()


def live_delay_ms():
    return OpenfoodfactsTestRunner.live_delay_ms()
