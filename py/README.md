...
The Python SDK for the Openfoodfacts API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Product(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Openfoodfacts(None).load(
    {"id": "test01"}, None
)
