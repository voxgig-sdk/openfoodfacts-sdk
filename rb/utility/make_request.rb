...
module OpenfoodfactsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = OpenfoodfactsResponse.new({})
    ...
    if fetch_err
      ...
      response = OpenfoodfactsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = OpenfoodfactsResponse.new(fetched)
    else
