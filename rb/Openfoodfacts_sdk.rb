...
require_relative 'Openfoodfacts_types'


class OpenfoodfactsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = OpenfoodfactsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = OpenfoodfactsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, OpenfoodfactsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    OpenfoodfactsUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = OpenfoodfactsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = OpenfoodfactsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = OpenfoodfactsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => OpenfoodfactsError.new(
        "#{op}_allow",
        "OpenfoodfactsSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = OpenfoodfactsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = OpenfoodfactsError.new(
        "graphql_error", "OpenfoodfactsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = OpenfoodfactsSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
