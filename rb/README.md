...
The Ruby SDK for the Openfoodfacts API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Product(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.Openfoodfacts(nil).load(
  { "id" => "test01" }, nil
)
...
```
rb/
├── Openfoodfacts_sdk.rb       -- Main SDK module
