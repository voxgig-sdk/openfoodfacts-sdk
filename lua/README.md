...
The Lua SDK for the Openfoodfacts API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Product(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Openfoodfacts(nil):load(
  { id = "test01" }, nil
)
