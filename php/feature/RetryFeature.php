...
// Openfoodfacts SDK retry feature
...
class OpenfoodfactsRetryFeature extends OpenfoodfactsBaseFeature
{
    ...
    public function init(OpenfoodfactsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (OpenfoodfactsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(OpenfoodfactsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(OpenfoodfactsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
