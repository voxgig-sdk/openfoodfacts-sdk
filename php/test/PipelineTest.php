...
// Openfoodfacts SDK pipeline test
//
...
{
    private static function utility(): OpenfoodfactsUtility
    {
        return new OpenfoodfactsUtility();
    }
    ...
    private static function ctx(array $over = []): OpenfoodfactsContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new OpenfoodfactsContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new OpenfoodfactsOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof OpenfoodfactsError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new OpenfoodfactsOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = OpenfoodfactsMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new OpenfoodfactsOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = OpenfoodfactsMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new OpenfoodfactsOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = OpenfoodfactsMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenfoodfactsSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = OpenfoodfactsMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new OpenfoodfactsResponse([]),
            'result' => new OpenfoodfactsResult([]),
        ]);
        [, $err] = OpenfoodfactsMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new OpenfoodfactsSpec([]),
            'response' => null,
            'result' => new OpenfoodfactsResult([]),
        ]);
        [, $err] = OpenfoodfactsMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new OpenfoodfactsSpec(['step' => 's']),
            'response' => new OpenfoodfactsResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new OpenfoodfactsResult([]),
        ]);
        [, $err] = OpenfoodfactsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenfoodfactsSpec(['step' => 's']),
            'response' => new OpenfoodfactsResponse(self::resp(200, ['v' => 1])),
            'result' => new OpenfoodfactsResult([]),
        ]);
        [, $err] = OpenfoodfactsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenfoodfactsSpec(['step' => 's']),
            'response' => new OpenfoodfactsResponse(self::resp(200, ['v' => 2])),
            'result' => new OpenfoodfactsResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        OpenfoodfactsMakeResponse::call($ctx);
    ...
    {
        $preset = new OpenfoodfactsResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new OpenfoodfactsSpec([]),
            'response' => new OpenfoodfactsResponse([]),
            'result' => new OpenfoodfactsResult([]),
        ...
        [$got, $err] = OpenfoodfactsMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new OpenfoodfactsResult([])]);
        [, $err] = OpenfoodfactsMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new OpenfoodfactsSpec([]), 'result' => null]);
        [, $err] = OpenfoodfactsMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenfoodfactsSpec(['step' => 's']),
            'result' => new OpenfoodfactsResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new OpenfoodfactsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenfoodfactsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenfoodfactsSpec(['step' => 's']),
            'result' => new OpenfoodfactsResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new OpenfoodfactsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenfoodfactsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenfoodfactsResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new OpenfoodfactsSpec([]),
            'result' => new OpenfoodfactsResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = OpenfoodfactsMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenfoodfactsMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new OpenfoodfactsError('boom', 'boom');
        $utility->fetcher = function (OpenfoodfactsContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenfoodfactsSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = OpenfoodfactsMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (OpenfoodfactsContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = OpenfoodfactsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(OpenfoodfactsResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        OpenfoodfactsMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (OpenfoodfactsContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenfoodfactsSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = OpenfoodfactsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new OpenfoodfactsResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new OpenfoodfactsSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = OpenfoodfactsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = OpenfoodfactsMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = OpenfoodfactsMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new OpenfoodfactsResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, OpenfoodfactsDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new OpenfoodfactsResult(['ok' => false])]);
        $this->expectException(OpenfoodfactsError::class);
        OpenfoodfactsDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new OpenfoodfactsResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', OpenfoodfactsMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        OpenfoodfactsMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new OpenfoodfactsBaseFeature();
        $b = new OpenfoodfactsBaseFeature();
        OpenfoodfactsFeatureAdd::call($ctx, $a);
        OpenfoodfactsFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): OpenfoodfactsBaseFeature
    {
        $f = new OpenfoodfactsBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        OpenfoodfactsFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?OpenfoodfactsSpec $spec): OpenfoodfactsContext
    {
    ...
    private static function auth_bags(): OpenfoodfactsSpec
    {
        return new OpenfoodfactsSpec(['headers' => [], 'query' => []]);
    }
    ...
    private static function auth_probe(array $options): ?array
    {
        ...
        OpenfoodfactsPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenfoodfactsPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new OpenfoodfactsResponse(['headers' => null]),
            'result' => new OpenfoodfactsResult([]),
        ]);
        OpenfoodfactsResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        OpenfoodfactsResultBody::call($ctx);
