...
// Openfoodfacts SDK Product entity
...
class ProductEntity
{
    ...
    {
        if ($args) {
            $this->_data = OpenfoodfactsHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = OpenfoodfactsHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   SDK boundary; throws OpenfoodfactsError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = OpenfoodfactsHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *   the SDK boundary; throws OpenfoodfactsError on failure (item-5 convention).
     */
