...
import (
	...
	omni "github.com/voxgig-sdk/openfoodfacts-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.OpenfoodfactsError); is {
				out := map[string]any{
...
func omniCtx(arg any, client *sdk.OpenfoodfactsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.OpenfoodfactsSDK); is {
			client = live
