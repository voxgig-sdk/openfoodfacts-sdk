...
[MCP](https://modelcontextprotocol.io) server exposing the Openfoodfacts SDK as
two agent tools — `openfoodfacts_list` and `openfoodfacts_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/openfoodfacts-mcp)
make build
...
claude mcp add --scope user openfoodfacts \
  -- /absolute/path/to/openfoodfacts-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./openfoodfacts-mcp -transport http -addr :8080
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/openfoodfacts-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user openfoodfacts \
     -- "$PWD"/dist/*/openfoodfacts-mcp -transport stdio
   ```
...
### Call the `openfoodfacts_list` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `openfoodfacts_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `openfoodfacts_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `OPENFOODFACTS_APIKEY` | API key sent with every request. |
| `OPENFOODFACTS_BASE` | Optional override of the API base URL. |
...
```sh
./openfoodfacts-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"openfoodfacts_load","arguments":{"entity":"product","query":{"id":1}}}}'
```
