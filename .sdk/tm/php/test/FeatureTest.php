...
// Openfoodfacts SDK feature test
//
...
class FtCtrl extends OpenfoodfactsControl
{
...
class FtClient
{
    ...
    private OpenfoodfactsUtility $utility;

    public function __construct(OpenfoodfactsUtility $utility, array $options)
    {
    ...
    public function get_utility(): OpenfoodfactsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OpenfoodfactsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public OpenfoodfactsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (OpenfoodfactsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OpenfoodfactsConfig::make_feature($name);
    ...
    private static function buildUrl(OpenfoodfactsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OpenfoodfactsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OpenfoodfactsResponse($fetched);
        }
    ...
    private function fail(OpenfoodfactsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OpenfoodfactsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
