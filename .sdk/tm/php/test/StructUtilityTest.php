...
// Openfoodfacts SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = OpenfoodfactsOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(OpenfoodfactsOmni::structfix($now, Struct::undef()))
                        !== json_encode(OpenfoodfactsOmni::structfix($val, Struct::undef()))) {
