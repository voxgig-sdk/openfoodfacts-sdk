...
//     (\`new OpenfoodfactsSDK(...)\` / \`OpenfoodfactsSDK.test(...)\` become
//     \`OpenfoodfactsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'OpenfoodfactsSDK'
...
const TEST_SEED = {"entity":{"product":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new OpenfoodfactsSDK(...)\` and any
// \`OpenfoodfactsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+OpenfoodfactsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/OpenfoodfactsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
