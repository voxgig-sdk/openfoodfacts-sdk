...
boru-driven command-line client **and** interactive REPL for the Openfoodfacts
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/openfoodfacts-cli)
make build

# 2. See usage (words, entities, env vars)
./openfoodfacts-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/openfoodfacts-cli
   ```
...
```sh
./openfoodfacts-cli load 1 product          # scalar shorthand for {id:1}
./openfoodfacts-cli load '{id:1}' product     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `openfoodfacts>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/openfoodfacts-cli`. |
...
(`./openfoodfacts-cli <expr>`) and interactively (the REPL), and expressions compose the
